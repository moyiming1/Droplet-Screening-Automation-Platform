function recordExperiment(expPath, sampleIndex)
%% Version history
% v0.1: Created by Yiming Mo 01/20/2019
%
%% Input: 
% expPath: experiment folder
% sampleIndex: which experiment is finished
...
for i = 1:length(header)
    title=header(i);
    if strcmp(title{1},'Finished?')
        column_num = i;
    end
end

cellTowrite = strcat(xlscol(column_num),num2str(sampleIndex+2));

xlswrite(fileName, 1, 1, cellTowrite)
